...
Не переносите старый локальный `.env`, где могли остаться переменные backend, базы данных, SMTP или Telegram bot.
...
- Контакты ведут только на `mailto:hello@tekyon.ru` и `https://t.me/czzttt`.
